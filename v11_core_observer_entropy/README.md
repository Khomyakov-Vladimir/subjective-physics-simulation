...
- **Documentation Split**  
  - `subjective_physics_simulation_v11.0_main_article.pdf` — main consolidated article  
  - `subjective_physics_simulation_v11.0_extended_notes.pdf` — supplementary material (retrodiction, weak values, EEG analogies, cultural variation)  
...
The **core** consists of a minimal set of scripts and figures, sufficient to reproduce  
the main results of the article (`subjective_physics_simulation_v11.0_main_article.pdf`):

- **Entropy scaling and RT distributions (from v5.0)**  
  `simulate_entropy_rt_full.py` →  
  `figures/histogram_rt.pdf`, `figures/scatter_entropy_rt.pdf`, `figures/cdf_rt.pdf`

- **Σ-projection filtering (from v9.0)**  
  `cognitive_decoherence_with_sigma.py` →  
  `figures/sigma_projection_result.pdf`, `figures/cognitive_filter_results.pdf`

- **Multi-agent Shared Reality Index (from v10.0)**  
  `cognitive_decoherence_with_sigma_multi_agent_system.py` →  
  `figures/sri_time_series.pdf`, `figures/shared_overlap_matrix.pdf`, `figures/agent_trajectories_pca.pdf`

These three modules constitute the **core workflow of version 11.0 (main article)**.
Running them in sequence is sufficient to reproduce the **main results of the article**, while the remaining figures of the article rely on the other scripts included in this repository.
...
These **extensions (v11.0_extended_notes)** are not required to reproduce the article’s results (v11.0_main_article), but are preserved to ensure full scientific transparency and repeatability.
...
| **v3.0** | (same as v2.0) | `trade_off_functional_lambda_comparison.py` → `L_epsilon_lambda_comparison_plot.pdf`; `landauer_extension.py` → `deltaS_vs_epsilon.pdf`, `energy_vs_epsilon.pdf`; `adaptive_perceptual_dynamics.py` → `adaptive_perceptual_dynamics.pdf` |
...
| **v8.0** | (inherits v5.0 core) | Cultural / contextual extensions (not central to core) |
| **v9.0** | `cognitive_decoherence_with_sigma.py` → `sigma_projection_result.pdf`, `cognitive_filter_results.pdf` | EEG-inspired modules → `subjective_phase_portrait.pdf`, `cognitive_trajectories.pdf`, `cog_reconstruction_noise.pdf` |
...
## Master Reproducibility Workflow (v1.0 → v11.0)

For full reproducibility, the scripts can be executed in the following order.  
This sequence reflects the historical development of the model,  
and ensures that all figures reported across versions are generated.

### Core Evolution
1. **v1.0 / v2.0**  
   `main.py` → basic entropy scaling (`entropy_vs_epsilon.pdf`, `norm_vs_time.pdf`, `trace_distance_vs_epsilon.pdf`)  
   (+ adaptive mode in v2: `adaptive_*` plots)

2. **v3.0**  
   `trade_off_functional_lambda_comparison.py` → `L_epsilon_lambda_comparison_plot.pdf`  
   `landauer_extension.py` → `deltaS_vs_epsilon.pdf`, `energy_vs_epsilon.pdf`  
   `adaptive_perceptual_dynamics.py` → `adaptive_perceptual_dynamics.pdf`

3. **v4.0**  
   `compare_entropy_fixed_adaptive.py` → `entropy_comparison.pdf`, `entropy_over_time.pdf`, `epsilon_over_time.pdf`, `trace_distance_over_time.pdf`

4. **v5.0**  
   `simulate_entropy_rt_full.py` → `histogram_rt.pdf`, `scatter_entropy_rt.pdf`, `cdf_rt.pdf`  
   *(baseline minimal model established)*

5. **v9.0**  
   `cognitive_decoherence_with_sigma.py` → `sigma_projection_result.pdf`, `cognitive_filter_results.pdf`

6. **v10.0**  
   `cognitive_decoherence_with_sigma_multi_agent_system.py` → `sri_time_series.pdf`, `shared_overlap_matrix.pdf`, `agent_trajectories_pca.pdf`

7. **v11.0 (Core)**  
   Running the three scripts above in sequence (`v5.0` + `v9.0` + `v10.0`)  
   reproduces the **main article (v11.0_main_article.pdf)**.

### Extensions (Optional, for completeness)
- v5.0: `compare_dirichlet_params.py` → distribution comparisons  
- v6.0: geodesic and trajectory modules  
- v7.0: retrodiction and weak values  
- v8.0: cultural/contextual extensions  
- v9.0: EEG-inspired modules  
- v10.0: parameter studies  
- v11.0: consolidated into `v11.0_extended_notes.pdf`
