...
- **v9_dynamic_phase_portrait/** — Introduces entropy-driven cognitive phase space simulation with stochastic jumps, EEG feedback, and fluctuation-theorem compliance:
  - `phase_portrait.py` — simulates 3D/4D trajectories with Tsallis entropy, entropy gradient dynamics, stochastic cognitive jumps with ΔE and \( P_+/P_- \) annotations, and EEG synchronization
...
- **v11_core_observer_entropy/** — Consolidates the framework into a minimal, self-contained formalism unifying entropy scaling, Σ-projection, and multi-agent dynamics. Provides:
  - Core definitions: projection operator \(F_\epsilon\), observer entropy \(S(\epsilon)\), and trade-off functional \(L(\epsilon)\).
  - Numerical experiments: entropy scaling, adaptive thresholds, RT distributions, and convergence in multi-agent settings.

- **Documentation Split**  
  - [Main article (v11.1, PDF)](https://github.com/Khomyakov-Vladimir/subjective-physics-simulation/releases/download/v11.1/subjective_physics_simulation_v11.1_main_article.pdf) — consolidated article  
  - [Extended notes (v11.1, PDF)](https://github.com/Khomyakov-Vladimir/subjective-physics-simulation/releases/download/v11.1/subjective_physics_simulation_v11.1_extended_notes.pdf) — supplementary material (retrodiction, weak values, EEG analogies, cultural variation) 
...
To install all required dependencies for **all published versions (v1–v11.1)** of the article:
...
All scripts in versions v1–v11.0 are fully reproducible using the following Conda environment:
