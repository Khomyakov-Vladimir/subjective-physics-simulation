...
Proven Cognitive Uncertainty Principle
--------------------------------------

Version 12.0 **proves** the _Cognitive Uncertainty Principle_ via information geometry:

∆ε · ∆KL ≥ C,   where   C = ½ minε ℐ(ε)−½

derived from the **Cramér–Rao bound** and **Fisher information** ℐ(ε) of the perceptual distribution. This establishes a _quantitative lower limit_ on simultaneous precision of perceptual resolution (∆ε) and statistical conformity (∆KL).

Observer Entropy Scaling
------------------------

Observer entropy under resolution ε follows the **exact scaling law**:

Sobs(ε) = log Cε ≈ log(1 + 1/ε),

verified numerically across 104 samples. This is _not an approximation_ but the entropy of the coarse-grained equivalence classes induced by the projection operator Fε.

Dynamic Entropy Law
-------------------

Observer entropy evolves by the **Ornstein–Uhlenbeck SDE**:

dSobs(t) = −κ(Sobs(t) − S\*) dt + σ dWt,

yielding a stationary Gaussian distribution centred at the intrinsic cognitive equilibrium S\*. Coupled multi-agent systems synchronize when coupling δ exceeds the critical value δc = κ / λmax(W).

Σ-Projection & Shared Reality Index
-----------------------------------

Multi-agent consensus is quantified by the **Shared Reality Index** (SRI):

SRI(t) = 1 − Vara(μ(a)(t)) / Varmax,

where μ(a)(t) = Σi i wi(a)(t) is the expected state index of agent a. Convergence is proven for the dynamics

ẇi(a) = −α Σb≠a |wi(a) − wi(b)| wi(a) + γ/(M−1) Σb≠a (wi(b) − wi(a)).

Thermodynamic Bound
-------------------

Perceptual discrimination cost is bounded by **Landauer's principle**:

Edisc(ε) = kBT ln 2 · log2(1/ε).

The observer optimally trades off information gain against energy via the functional

ℒ(ε) = Sobs(ε) − λE Edisc(ε),

minimised at ε\*(λE) = (λE kBT ln 2)−1 for the logarithmic entropy approximation.

Reproducibility
---------------

All figures and results are generated by the following **deterministic Python scripts** (seed = 42):

*   `simulate_entropy_dynamics.py` – integrates the SDE and produces entropy trajectories.
*   `generate_entropy_phase_map.py` – maps the (α, β) phase diagram.
*   `simulate_multiagent_synchronization.py` – solves the coupled ODE system and outputs SRI(t).
...
*   [**A Technical Summary of Subjective Physics (v12.0)**](https://doi.org/10.5281/zenodo.17407408) — a concise overview of the framework, key equations, and simulation protocols ([PDF](https://zenodo.org/records/17407408/files/a-technical-summary-of-subjective-physics-v12.0-2025.pdf)).
*   [**Extended Notes (v11.2)**](https://doi.org/10.5281/zenodo.17407408) — supplementary discussions on retrodiction, weak values, EEG-inspired portraits, and cultural perspectives ([PDF](https://zenodo.org/records/17407408/files/subjective_physics_simulation_v11.2_extended_notes.pdf)).
